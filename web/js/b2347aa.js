...
$(document).ready(function(){        
    ...
    $('.navigation i.fa-envelope').parents('li').bind('click',function(){
        $.get('/contact/index',[],function(data){
            $('#modal-contact-form .modal-body').html(data);
            $('#modal-contact-form').modal({show: true});
        })
    })
...
function page_navigation_close(mode,width){
    $('.page-navigation-toggle').css('display','block');
    if(!mode){ //Desktop mode
...
function page_sidebar(action){
    $('.page-navigation-toggle').css('display','none');
